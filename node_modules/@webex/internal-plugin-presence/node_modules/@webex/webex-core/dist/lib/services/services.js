  ...
  updateServices: function updateServices() {
    ...
    if (serviceGroup === 'preauth') {
      ...
      if (!['email', 'userId', 'orgId', 'mode'].includes(queryKey)) {
        return _promise2.default.reject(new Error('a query param of email, userId, orgId, or mode is required'));
      }
    }
    ...
    if (serviceGroup === 'preauth' || serviceGroup === 'signin') {
      ...
      formattedQuery = {};
      formattedQuery[_queryKey] = _queryKey === 'email' ? _common.base64.encode(query.email) : query[_queryKey];
    }
